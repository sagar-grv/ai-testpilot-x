...
def test_requirement_schema():
    r = RequirementSchema(
        modules=["Login"], risk_areas=["Auth"], priority="High", raw_input="test"
    )
...
def test_verification_schema():
    v = VerificationSchema(
        coverage_score=0.9, duplicate_count=0, missing_edge_cases=[], passed=True
    )
...
def test_bug_schema():
    b = BugSchema(
        ...
        severity="High",
        priority="P2",
        failure_signature="NoSuchElement_login",
        root_cause="Selector changed",
        root_cause_confidence=0.9,
        fix_suggestion="Update selector",
        fix_confidence=0.8,
...
def test_bug_cluster_schema():
    c = BugClusterSchema(
        cluster_id="C1",
        root_cause_summary="Timeout issues",
        bug_ids=["BUG-1"],
        severity="High",
        count=1,
    )
...
def test_report_schema():
    ...
    r = ReportSchema(
        ...
        recommendation_text="Risk present",
        risk_score=45.0,
