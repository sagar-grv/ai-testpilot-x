...
def bugs_cmd(
    log: Optional[str] = typer.Option(
        None, "--log", "-l", help="Error log text or path to a .log/.txt file."
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Save bug report JSON to this file."
    ),
    config: Optional[Path] = typer.Option(
        None, "--config", "-c", help="Path to testpilot.yaml."
    ),
    session: Optional[str] = typer.Option(
        None, "--session", help="Session ID for RAG correlation."
    ),
):
    ...
    if bug.rag_matches:
        console.print()
        console.print(
            f"[bold]Similar past bugs (RAG):[/bold]  {len(bug.rag_matches)} matches"
        )
