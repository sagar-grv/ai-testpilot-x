...
def run_cmd(
    story: str = typer.Option(..., "--story", "-s",
        help="Plain-English user story to test."),
    url: Optional[str] = typer.Option(None, "--url", "-u",
        help="Target application URL (overrides testpilot.yaml)."),
    config: Optional[Path] = typer.Option(None, "--config", "-c",
        help="Path to testpilot.yaml (default: auto-detect in cwd)."),
    mode: Optional[str] = typer.Option(None, "--mode", "-m",
        help="Execution mode: MOCK | LOCAL | GRID"),
    output: Optional[Path] = typer.Option(None, "--output", "-o",
        help="Save full report JSON to this file."),
    ...
    with Progress(
        ...
        def _update(stage: str, done: bool = False, extra: str = ""):
            if done:
                progress.update(task, description=f"[green]✓[/green]  {stage}  [dim]{extra}[/dim]")
            else:
    ...
    BANNERS = {
        ...
        "GO_WITH_RISK": ("yellow", "⚠️ ", "GO WITH RISK", "High severity issues present."),
