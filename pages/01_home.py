...
gemini_ok = bool(
    settings.GEMINI_API_KEY and settings.GEMINI_API_KEY.startswith("AIzaSy")
)
with h1:
    st.markdown(
        _health_card(
...
with h3:
    st.markdown(
        _health_card(
            "⚙️", "Execution Engine", settings.EXECUTION_MODE, exec_detail, exec_ok
        ),
        unsafe_allow_html=True,
    )
...
# ── Last Run Summary ──────────────────────────────────────────────────────────
st.markdown(
    '<div class="tp-section-title">Last Run Summary</div>', unsafe_allow_html=True
)
...
if rdata:
    ...
    k4.metric(
        ...
        sum(
            rdata.get(k, 0)
            for k in ["critical_bugs", "high_bugs", "medium_bugs", "low_bugs"]
        ),
    )
...
# ── Agent Activity Feed ───────────────────────────────────────────────────────
st.markdown(
    '<div class="tp-section-title">Agent Activity Feed</div>', unsafe_allow_html=True
)
...
if history:
    for ev in reversed(history[-12:]):
        ...
        payload_str = (
            " · ".join(f"{k}: {v}" for k, v in list(payload.items())[:3])
            if payload
            else ""
        )
        st.markdown(
