...
st.set_page_config(page_title="AI Test Generator | AI TestPilot X", page_icon="🤖", layout="wide")
...
if st.session_state.get("analysis_done") and (req := st.session_state.get("requirement_context")):
    st.divider()
    st.markdown("""
    ...
    if st.session_state.get("testcases_done") and (tcs := st.session_state.get("generated_testcases")):
        # Summary banner
        type_counts = {}
        for tc in tcs:
        ...
        TYPE_COLORS = {
            ...
            "Security": "#f59e0b", "Performance": "#3b82f6",
            "Boundary": "#ec4899", "Accessibility": "#a855f7"
        }
        PRIO_COLORS = {
        ...
        try:
            ...
            cats = ["Positive", "Negative", "Security", "Performance", "Boundary", "Accessibility"]
            ...
            fig.update_layout(
                polar=dict(
                    radialaxis=dict(visible=True, showticklabels=False, gridcolor="#1E2337"),
                    angularaxis=dict(gridcolor="#1E2337"),
                    bgcolor="#0F1117"
                ),
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(color="#94A3B8", size=12),
                title=dict(text="Coverage Radar", font=dict(size=13, color="#94A3B8"), x=0.5),
                height=300, margin=dict(l=50, r=50, t=40, b=20), showlegend=False
            )
        ...
        if v_rep := st.session_state.get("verification_report"):
            ...
            vrow1.metric(
                "Coverage Score", f"{pct}%",
                delta="✓ Pass" if passed else "Below 60% threshold"
            )
            vrow2.metric("Duplicates Found", v_rep.duplicate_count,
                         delta="None detected" if v_rep.duplicate_count == 0 else "Review needed")
        ...
        with ex1:
            ...
            st.download_button("⬇ Export CSV", csv, "test_cases.csv",
                               "text/csv", use_container_width=True)
        with ex2:
            if st.button("▶ Go to Selenium Generator →", type="primary", use_container_width=True):
                st.success("✅ Test cases saved to session. Open **Selenium Generator** in the sidebar.")
