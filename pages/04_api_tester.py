...
with c_mode:
    mode = st.radio("Execution Mode", ["Mock (Generate Only)", "Live (Execute)"],
...
with c_mod:
    module_options = req.modules if req and hasattr(req, "modules") else ["Login", "Cart", "Checkout", "Payment"]
    selected_module = st.selectbox("Module", module_options)
with c_btn:
    st.markdown("<br>", unsafe_allow_html=True)
    gen_btn = st.button("🔄 Generate API Tests", type="primary", use_container_width=True)
...
if suite:
    ...
    if any(k in module_lower for k in ["pay", "checkout", "billing", "card", "stripe"]):
        risk, risk_bg, risk_border, risk_color = "CRITICAL", "#450a0a", "#ef4444", "#f87171"
    elif any(k in module_lower for k in ["login", "auth", "token", "password", "session"]):
        risk, risk_bg, risk_border, risk_color = "HIGH", "#431407", "#f97316", "#fb923c"
    else:
        risk, risk_bg, risk_border, risk_color = "MEDIUM", "#422006", "#f59e0b", "#fbbf24"
    ...
    df = pd.DataFrame([{
        ...
        "Assertion": t.assertion[:60] + ("..." if len(t.assertion) > 60 else ""),
    } for t in suite])
    ...
    if live_mode and base_url:
        ...
        if st.button("▶ Run Live Tests", type="primary"):
            ...
            for i, test in enumerate(suite):
                ...
                prog.progress((i + 1) / len(suite), text=f"Running {i+1}/{len(suite)}...")
            prog.empty()
            ...
            rdf = pd.DataFrame([{
                ...
                "Expected": r.expected_status, "Actual": r.actual_status,
    ...
    with exp2:
        ...
        collection = {
            "info": {"name": f"AI TestPilot — {selected_module}",
                     "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"},
            "item": [{"name": f"{t.method} {t.endpoint}", "request": {
                "method": t.method, "url": f"{{{{base_url}}}}{t.endpoint}",
