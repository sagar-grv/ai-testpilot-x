...
if view == "🏢 Executive":
    try:
        ...
        with chart1:
            st.markdown('<div class="tp-section-title">Test Results</div>',
                        unsafe_allow_html=True)
            ...
            if total > 0:
                ...
                fig.update_layout(
                    ...
                    annotations=[dict(
                        ...
                        x=0.5, y=0.5, font=dict(size=16, color="#F0F4FF"),
                        showarrow=False
                    )]
                )
        ...
        with chart2:
            st.markdown('<div class="tp-section-title">Bug Severity Breakdown</div>',
                        unsafe_allow_html=True)
    ...
    # Bug summary table
    if bugs:
        st.markdown('<div class="tp-section-title">Bug Summary</div>', unsafe_allow_html=True)
        SEV_ICONS = {"Critical": "🔴", "High": "🟠", "Medium": "🟡", "Low": "🟢"}
        bug_list = bugs if isinstance(bugs[0], dict) else [b.model_dump() for b in bugs]
        rows = [{
            "": SEV_ICONS.get(b.get("severity",""), "⚪"),
            "ID": b.get("id",""),
            "Title": b.get("title","")[:55] + ("..." if len(b.get("title","")) > 55 else ""),
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # Test results detail
    st.markdown('<div class="tp-section-title">Test Results Detail</div>', unsafe_allow_html=True)
    if execution:
    ...
    # Bug detail cards
    st.markdown('<div class="tp-section-title">Bug Analysis</div>', unsafe_allow_html=True)
    if bugs:
        bug_list2 = bugs if isinstance(bugs[0], dict) else [b.model_dump() for b in bugs]
        SEV_STYLES2 = {
