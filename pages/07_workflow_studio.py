...
st.set_page_config(
    page_title="Workflow Studio | AI TestPilot X", page_icon="🔀", layout="wide"
)
...
PIPELINE = [
    (
        ...
        "Parses user story into modules & risk areas",
    ),
    (
        "📝",
        "TestCaseAgent",
        "TESTCASES_GENERATED",
        "Generates structured test cases with RAG",
    ),
    (
        "✅",
        "VerificationAgent",
        "VERIFICATION_COMPLETE",
        "Checks coverage & edge case gaps",
    ),
...
st.markdown(
    '<div class="tp-section-title">Pipeline Status</div>', unsafe_allow_html=True
)
...
# ── Agent flow diagram ────────────────────────────────────────────────────────
st.markdown(
    '<div class="tp-section-title">Agent Flow Diagram</div>', unsafe_allow_html=True
)
...
try:
    ...
    nodes = [
        Node(
            ...
            label=f"{icon} {name}",
            color={
                "background": _node_color(name, ev),
...
st.markdown(
    '<div class="tp-section-title">GlobalState Inspector</div>', unsafe_allow_html=True
)
st.markdown(
    """
...
if ctrl1.button("💾 Save Checkpoint", use_container_width=True):
    st.session_state["checkpoint"] = {
        k: st.session_state.get(k) for k, _, _ in STATE_MAP
    }
