...
class BugMemory:
    ...
    def store_bug(self, session_id: str, bug_id: str, title: str, root_cause: str,
                  failure_signature: str, severity: str) -> None:
        try:
            from core.rag_engine import rag_engine
            text = f"{title} {root_cause} {failure_signature}"
            rag_engine.upsert("bugs", bug_id, text, {"severity": severity, "session_id": session_id})
        except Exception as e:
