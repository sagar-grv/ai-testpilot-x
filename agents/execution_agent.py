...
class ExecutionAgent(BaseAgent):
    ...
    def prepare_execution_plan(self, scripts: dict, target_url: str) -> str:
        lines = [
            f"Execution Plan — Target: {target_url}",
            f"Test scripts: {len(scripts)}",
        ]
    ...
    def run(
        self, mode: str, scripts: dict, target_url: str, approved: bool
    ) -> ExecutionSchema:
        if not approved:
    ...
    def _run_mock(self, scripts: dict) -> ExecutionSchema:
        ...
        bus.emit(
            ...
            {"total": schema.total, "failed": schema.failed},
        )
        return schema
    ...
    def _run_local(self, scripts: dict, target_url: str) -> ExecutionSchema:
        ...
        for tc_id, code in scripts.items():
            result = run_test(
                tc_id, code, target_url, headless=settings.SELENIUM_HEADLESS
            )
    ...
    def _run_grid(self, scripts: dict, target_url: str) -> ExecutionSchema:
        ...
        for tc_id, code in scripts.items():
            ...
            try:
                ...
                results.append(
                    TestResultSchema(
                        tc_id=tc_id, status="PASS", duration_ms=duration_ms
                    )
                )
            except Exception as e:
                duration_ms = (time.perf_counter() - start) * 1000
